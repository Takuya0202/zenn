...
## 11章(命名)
### クラス等を命名するうえでのポイント
- `目的`に沿った名前をつけ、なるべく具体的で意味の狭い名前にする。抽象的・広義な名前にすると、関心の異なるものが混入しやすくなり、変更時の影響範囲も広がってしまう。
- 一般的な言葉よりも業務上の目的を表す言葉を優先する。「ユーザー」「クライアント」のような汎用的な単語より、「出品者」「購入者」のようにドメインの意図が伝わる言葉を使う。
- `xxxxData`・`xxxxInfo` のようなデータクラスを連想させる名前は避ける。また `xxxxManager` のような抽象的な名前は責務が曖昧になり、関心の異なるロジックが混在しやすいため避ける。

## 14章(モデリング)
`モデル`とはシステム構造を説明・理解しやすいよう図式化したもの。
### モデルの作り方
モデルはシステムの目的達成のための手段の一つであり、**特定の目的に対して最小限の要素**を備えたものである。
つまり、目的を明確にしない限り最小限の構成要素が定まらず、複数の目的が混在した一貫性のないモデルになってしまう。
これを踏まえると、物理的な世界では利用者が一人であっても、その利用者がやりたいことは複数の目的にまたがるため、`User` という単一の概念に対しても複数のモデルが存在することになる。
