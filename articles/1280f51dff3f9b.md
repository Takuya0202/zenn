...
この設計では、`auth.users`テーブルで認証情報（email、password）を管理し、`User`テーブルでアプリケーション固有のユーザー情報（name、userIcon）を管理します。両テーブルは`id`（UUID）で1対1の関係で結ばれています。
...
これはサーバーでの操作なので`server.ts`からインポートします。
