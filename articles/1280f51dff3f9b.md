...
:::message
一応`options`をつけて`auth.users`に保存する方法もありますが、ユーザー情報が大きくなってしまった場合を考慮して上記の方法が良いと思いました。
:::

## データベース設計について
今回は以下のような方法でテーブル設計をしました。
```mermaid
erDiagram
    auth_users {
        uuid id PK
    ...
    auth_users ||--|| User : "1:1 relationship"
```
...
この設計では、`auth.users`テーブルで認証情報（email、password）を管理し、
`User`テーブルでアプリケーション固有のユーザー情報（name、userIcon）を管理します。
`User`テーブルは`auth.user`のidで登録します。
...
```
まず`const supabase = await createClient();`でクライアントを作成します。
次に`signUp`関数を使ってemailとpasswordでユーザー登録を行います。
登録が成功時、userIdは`data.user.id`で取得できるため、それを`User`テーブルの主キーとして
アカウント詳細情報を登録します。`signUp`の詳細は以下の通り。
